...
function dbFeeds(feedId) {
  return firebase.database()
      .ref('usersFeed')
      .child(feedId);
}

function dbUserFeed(userId) {
  return firebase.database()
...
function screenFeedInitialize(userId) {
  return (dispatch, getState) => {
    ...
    dbUserFeed(userId).on('child_added', (snapshot) => {
...
function screenFeedReceiveFeed(receivedFeedIdsArr) {
  return (dispatch, getState) => {
    ...
    stateFeedIdsArr.forEach(chunkOfFeedIds => {
      Promise.all(chunkOfFeedIds.map(feedObj => dbFeeds(feedObj).once('value').then(f => f.val())))
          .then(feedArr => {
