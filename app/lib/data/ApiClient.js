...
class ApiClient {
	...
	_handleResponse(response, url, callbackRetryRequest) {
		...
		if (response.status === 200) {
			...
			Logger.v(`ApiClient _handleResponse: status(200) for ${url}`);
	...
	_postMultipart(url, formData) {
		Logger.v(`ApiClient _postFile: Requesting auth-token for ${url}`);
		
		return ApiAuthentication.getUserAuthenticationToken()
			.then(authenticationToken => {
				Logger.v(`ApiClient _postFile: Using auth-token ${authenticationToken}`);
